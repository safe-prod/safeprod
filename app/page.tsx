    ...
    <div className="flex flex-col">
      ...
      {items.map((_, index) => (
        <div key={index} className="p-2 mb-2 rounded-lg flex flex-col bg-gray-200">
          <div className="m-2 flex flex-row grow font-bold text-xl">
          ...
          <div className="flex flex-row grow font-bold text-sm">
            <div className="text-red-900 grow">Sep 27, 5:03PM</div>
            <div className="rounded-2xl max-w-24 text-center bg-gray-300 text-red-900">Money</div>
          </div>
        </div>
      ))}
      <div className="fixed bottom-2 right-2 bg-red-700 text-white font-bold text-xl rounded-xl p-4 shadow-lg shadow-red-500">
        +
      </div>
    </div>
  )
